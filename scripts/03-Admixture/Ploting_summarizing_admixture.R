...
library(pophelper)
#--- File paths ---
qmatrix <- sprintf("data_out/03-Admixture/full_isolate/260109full__outfiles_K%d.qopt", c(6:20))
...
summarize_ancestries <- function(K, sample_file = "data_out/03-Admixture/full_isolate/pop.list") {
  
  # Read Q matrix
  qfile <- sprintf("data_out/03-Admixture/full_isolate/260109full__outfiles_K%d.qopt", K)
...
result_wide <- result_wide %>%
  left_join(metadata_1 %>% select(Accession, Proposed_final_clade),
            by = c("Sample" = "Accession"))


library(readxl)
X938_isolates_with_clade_singleChrPhylo <- read_excel("data_out/02-PhylogenyClade/Per_chr_clades/938_isolates_with_clade_singleChrPhylo.xlsx")
View(X938_isolates_with_clade_singleChrPhylo)
# Columns to add
chr_cols <- c("Sample","Isolate_name", "Chr1", "Chr2", "Chr3", "Chr4", "Chr5", "Chr6", "Chr7", "ChrR")

# Subset the chromosome table
chr_data <- X938_isolates_with_clade_singleChrPhylo %>%
  select(all_of(chr_cols))

result_wide_1 <- result_wide %>%
  left_join(chr_data, by = "Sample")

View(result_wide_1)
write.table(result_wide_1, "data_out/03-Admixture/260112_ancestries_prop_2.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
